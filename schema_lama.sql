...
CREATE TABLE public.kelas (
    ...
    id_pengajar INTEGER REFERENCES public.users(id) ON DELETE SET NULL,
...
CREATE TABLE public.jadwal_kelas (
    ...
    id_kelas INTEGER REFERENCES public.kelas(id_kelas) ON DELETE CASCADE,
    hari VARCHAR(50),
    jam_mulai TIME,
    jam_selesai TIME,
    link_pertemuan TEXT,
...
CREATE TABLE public.enrollment (
    ...
    id_siswa INTEGER REFERENCES public.users(id) ON DELETE CASCADE,
    id_kelas INTEGER REFERENCES public.kelas(id_kelas) ON DELETE CASCADE,
...
CREATE TABLE public.kelas_siswa (
    id_kelas_siswa SERIAL PRIMARY KEY,
    id_siswa INTEGER REFERENCES public.users(id) ON DELETE CASCADE,
    id_kelas INTEGER REFERENCES public.kelas(id_kelas) ON DELETE CASCADE
);
...
CREATE TABLE public.materi_audio (
    ...
    id_materi INTEGER REFERENCES public.materi(id_materi) ON DELETE CASCADE,
...
CREATE TABLE public.forum_diskusi (
    id_diskusi SERIAL PRIMARY KEY,
    id_kelas INTEGER REFERENCES public.kelas(id_kelas) ON DELETE CASCADE,
    id_penulis INTEGER REFERENCES public.users(id) ON DELETE CASCADE,
    judul VARCHAR(255),
    pesan TEXT,
    created_at TIMESTAMPTZ DEFAULT NOW()
);
...
CREATE TABLE public.forum_komentar (
    ...
    id_diskusi INTEGER REFERENCES public.forum_diskusi(id_diskusi) ON DELETE CASCADE,
    komentar TEXT
);
...
CREATE TABLE public.forum_chat (
    ...
    id_materi INTEGER REFERENCES public.materi(id_materi) ON DELETE CASCADE,
    id_user INTEGER REFERENCES public.users(id) ON DELETE CASCADE,
    pesan TEXT,
    created_at TIMESTAMPTZ DEFAULT NOW(),
    parent_id INTEGER REFERENCES public.forum_chat(id_chat) ON DELETE CASCADE
);
...
CREATE TABLE public.log_aktivitas_kuis (
    ...
    id_siswa INTEGER REFERENCES public.users(id) ON DELETE CASCADE,
    id_kuis INTEGER REFERENCES public.kuis(id_kuis) ON DELETE CASCADE
);
...
CREATE TABLE public.opsi_pg (
    id_opsi SERIAL PRIMARY KEY,
    id_soal INTEGER REFERENCES public.soal_kuis(id_soal) ON DELETE CASCADE,
    teks_opsi TEXT,
    is_benar BOOLEAN
);
...
CREATE TABLE public.sertifikat (
    id_sertifikat SERIAL PRIMARY KEY,
    id_siswa INTEGER REFERENCES public.users(id) ON DELETE CASCADE,
    nama_sertifikat VARCHAR(255),
    file_pdf VARCHAR(255),
    tanggal_keluar TIMESTAMPTZ,
    status_approve BOOLEAN DEFAULT false,
    id_kelas INTEGER REFERENCES public.kelas(id_kelas) ON DELETE SET NULL
);
